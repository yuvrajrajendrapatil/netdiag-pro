...
// ─── NetworkChart WITH HEARTBEAT/STOCK MARKET STYLE ─────────────────────────
function NetworkChart({ data, loading, autoUpdate = true }) {
  const [activePoint, setActivePoint] = useState(null);
  const [liveData, setLiveData] = useState({ labels: [], download: [], upload: [] });
  const updateInterval = useRef(null);
  const maxPoints = 30; // Last 30 points visible

  // ✅ HEARTBEAT ANIMATION - Moves Left to Right
  useEffect(() => {
    if (loading || !data) return;

    // Initialize with 15 points
    const initialLabels = [];
    ...
    for (let i = 0; i < 15; i++) {
      const now = new Date();
      now.setSeconds(now.getSeconds() - (15 - i) * 2);
      initialLabels.push(now.toLocaleTimeString('en-US', {
        hour12: false,
        hour: '2-digit',
        minute: '2-digit',
        second: '2-digit'
      }));
      initialDownload.push(rand(60, 95));
      initialUpload.push(rand(20, 40));
    }
    ...
    // ✅ UPDATE EVERY 2 SECONDS - CONTINUOUS FLOW
    updateInterval.current = setInterval(() => {
      setLiveData(prev => {
        const newLabels = [...prev.labels];
        const newDownload = [...prev.download];
        const newUpload = [...prev.upload];

        // Remove oldest (left side)
        ...
        // Add newest (right side)
        const now = new Date();
        newLabels.push(now.toLocaleTimeString('en-US', {
          hour12: false,
          hour: '2-digit',
          minute: '2-digit',
          second: '2-digit'
        ...
        // Smooth fluctuation (not jumpy)
        const lastDl = newDownload[newDownload.length - 1] || 80;
        const lastUl = newUpload[newUpload.length - 1] || 30;

        newDownload.push(Math.max(50, Math.min(100, lastDl + rand(-8, 8))));
        newUpload.push(Math.max(15, Math.min(50, lastUl + rand(-5, 5))));
  ...
  }, [data, loading, autoUpdate]);

  if (loading || liveData.labels.length === 0) return (
    <div style={card.wrap}>
    ...
    datasets: [
      {
        ...
        backgroundColor: (ctx) => {
          ...
          g.addColorStop(0, "rgba(0,245,196,0.25)");
      ...
      },
      {
        ...
        backgroundColor: (ctx) => {
          const g = ctx.chart.ctx.createLinearGradient(0, 0, 0, 320);
          g.addColorStop(0, "rgba(123,97,255,0.2)");
  ...
  const opts = {
    ...
    onClick: (_, els) => {
      ...
      const i = els[0].index;
      setActivePoint({
        time: liveData.labels[i],
        dl: liveData.download[i].toFixed(1),
        ul: liveData.upload[i].toFixed(1)
      });
    },
    plugins: {
      ...
      tooltip: {
        ...
        callbacks: {
          title: (i) => `⏱  ${i[0].label}`,
          label: (i) => ` ${i.dataset.label}: ${i.parsed.y.toFixed(1)} Mbps`
        },
      },
    },
    scales: {
            ...
            {autoUpdate && (
              <div style={{
                ...
                <div style={{
                  ...
                  animation: "ndPulse 1.5s ease infinite"
                }} />
                <span style={{
                  fontSize: 9,
                  fontWeight: 800,
      ...
      <div style={{
        ...
        <div style={{ flex: 1 }}>
          ...
          <div style={{
            ...
            {liveData.download[liveData.download.length - 1]?.toFixed(1) || "0"}
            <span style={{ fontSize: 12, marginLeft: 4 }}>Mbps</span>
          </div>
        </div>
        <div style={{ flex: 1 }}>
          <div style={{
          ...
          <div style={{
            ...
            {liveData.upload[liveData.upload.length - 1]?.toFixed(1) || "0"}
            <span style={{ fontSize: 12, marginLeft: 4 }}>Mbps</span>
          </div>
        </div>
      </div>
...
export default function Home() {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      {/* ✅ Main line chart WITH LIVE AUTO-UPDATE */}
      <NetworkChart data={chartData} loading={loading} autoUpdate={true} />
